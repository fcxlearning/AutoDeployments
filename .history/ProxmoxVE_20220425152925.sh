...
function SetRepositoryMirror() {
    mirror_list=(
        ...
        "deb ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename} main contrib non-free"
        "deb ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename}-backports main contrib non-free"
        "deb ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename}-updates main contrib non-free"
        "deb-src ${transport_protocol}://mirror.dal.nexril.net/debian-security ${LSBCodename}-security main contrib non-free"
        "deb-src ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename} main contrib non-free"
        "deb-src ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename}-backports main contrib non-free"
        "deb-src ${transport_protocol}://mirror.dal.nexril.net/debian ${LSBCodename}-updates main contrib non-free"
    )
    proxmox_mirror_list=(
        "# deb ${transport_protocol}://enterprise.proxmox.com/debian/pve ${LSBCodename} pve-enterprise"
        "deb ${transport_protocol}://mirror.dal.nexril.net/proxmox/debian ${LSBCodename} pve-no-subscription"
        "# deb ${transport_protocol}://mirror.dal.nexril.net/proxmox/debian ${LSBCodename} pvetest"
        "deb ${transport_protocol}://mirror.dal.nexril.net/proxmox/debian/ceph-pacific ${LSBCodename} main"
        "# deb ${transport_protocol}://mirror.dal.nexril.net/proxmox/debian/ceph-pacific ${LSBCodename} test"
    )
...
function ConfigurePackages() {
    function ConfigureChrony() {
        ...
        if [ "$?" -eq "0" ]; then
            rm -rf "/tmp/chrony.autodeploy" && for chrony_list_task in "${!chrony_list[@]}"; do
                ...
                if [ "${chrony_ntp_list[$chrony_ntp_list_task]}" == "time.cloudflare.com" ] || [ "${chrony_ntp_list[$chrony_ntp_list_task]}" == "cn.ntp.org.cn" ] || [ "$(echo ${DHCP_NTP[@]} | grep ${chrony_ntp_list[$chrony_ntp_list_task]})" != "" ]; then
    ...
    function ConfigurePythonPyPI() {
        ...
        if [ "${WHICH_PIP}" != "null" ]; then
            ${WHICH_PIP} config set global.index-url "https://mirror.dal.nexril.net/pypi/web/simple"
        fi
